package LaboratoryExercises.Lab06;
...
class University {
    ...
    public List<String> getAllCourseNames() {
        return departments.stream().flatMap(department -> department.getCourses().stream()).map(Course::getName).collect(Collectors.toList());
    }

    public List<Course> getCoursesWithMinCredits(int minCredits) {
        return departments.stream().flatMap(department -> department.getCourses().stream()).filter(course -> course.getCredits() >= minCredits).collect(Collectors.toList());
    }

    public int getTotalStudentCount() {
        return departments.stream().flatMap(department -> department.getCourses().stream()).mapToInt(Course::getEnrolledStudents).sum();
    }

    public Optional<Course> getHardestCourse() {
        return departments.stream().flatMap(department -> department.getCourses().stream()).max(Comparator.comparingInt(Course::getDifficulty));
    }

    public Map<Integer, List<Course>> groupByDifficulty() {
        return departments.stream().flatMap(department -> department.getCourses().stream()).collect(Collectors.groupingBy(Course::getDifficulty));
    }

    public Map<String, Integer> getCourseEnrollmentMap() {
        return departments.stream().flatMap(d -> d.getCourses().stream()).collect(Collectors.toMap(Course::getCode, Course::getEnrolledStudents));
    }

    public double getAverageEnrollmentPerCourse() {
        return getCourseEnrollmentMap().values().stream().mapToInt(Integer::intValue).average().orElse(0);
    }

    public List<String> getSortedCourseCodes() {
        return new TreeMap<>(getCourseEnrollmentMap()).keySet().stream().collect(Collectors.toList());
    }

    public Map<String, List<String>> getDepartmentToCourseNames() {
        return departments.stream().collect(Collectors.toMap(Department::getName, department -> department.getCourses().stream().map(Course::getName).collect(Collectors.toList())));
    }

    public List<Course> getAllCourses() {
        return departments.stream().flatMap(department -> department.getCourses().stream()).collect(Collectors.toList());
    }

    public Optional<Department> getMostPopularDepartment() {
        return departments.stream().max(Comparator.comparingInt(department -> department.getCourses().stream().mapToInt(Course::getEnrolledStudents).sum()));
    }

    public Map<Integer, Integer> getStudentsByDifficulty() {
        return departments.stream().flatMap(department -> department.getCourses().stream()).collect(Collectors.groupingBy(Course::getDifficulty, Collectors.summingInt(Course::getEnrolledStudents)));
    }

    public List<Course> getCoursesByDifficultyRange(int min, int max) {
        return departments.stream().flatMap(department -> department.getCourses().stream()).filter(course -> course.getDifficulty() >= min && course.getDifficulty() <= max).collect(Collectors.toList());
    }

    public List<String> getPopularCourseCodes(int minStudents) {
        return departments.stream().flatMap(department -> department.getCourses().stream()).filter(course -> course.getEnrolledStudents() >= minStudents).map(Course::getCode).collect(Collectors.toList());
    }

    public Map<String, Integer> getTotalCreditsPerDepartment() {
        return departments.stream().collect(Collectors.toMap(Department::getName, department -> department.getCourses().stream().mapToInt(Course::getCredits).sum()));
    }

    public List<Course> getTop3HardestCourses() {
        return departments.stream().flatMap(department -> department.getCourses().stream()).sorted(Comparator.comparingInt(Course::getDifficulty).reversed()).limit(3).collect(Collectors.toList());
    }


    public Map<String, Double> getAverageDifficultyPerDepartment() {
        return departments.stream().collect(Collectors.toMap(Department::getName, department -> department.getCourses().stream().mapToInt(Course::getDifficulty).average().orElse(0)));
    }


    public IntSummaryStatistics getEnrollmentStatistics() {
        return departments.stream().flatMap(department -> department.getCourses().stream()).mapToInt(Course::getEnrolledStudents).summaryStatistics();
    }


    public University mergeFourSmallestDepartments() {
        List<Department> sorted = departments.stream().sorted(Comparator.comparingInt(d -> d.getCourses().stream().mapToInt(Course::getEnrolledStudents).sum())).collect(Collectors.toList());
        List<Department> smallest = sorted.subList(0, 4);

        Department merged = smallest.stream().reduce((d1, d2) -> {
            String name = d1.getName() + " & " + d2.getName();
            List<Course> courses = Stream.concat(d1.getCourses().stream(), d2.getCourses().stream())
                    .collect(Collectors.toList());
            return new Department(name, courses);
        }).orElseThrow();

        List<Department> result = new ArrayList<>(sorted.subList(4, sorted.size()));
        result.add(merged);
