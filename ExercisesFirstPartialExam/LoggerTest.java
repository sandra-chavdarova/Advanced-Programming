...
class LogSystem<T extends ILog> {
    ...
    void printResults() {
        LogProcessor<T> firstLogProcessor = l -> l.stream().filter(i -> i.getType().equals("INFO")).collect(Collectors.toCollection(ArrayList::new));
        LogProcessor<T> secondLogProcessor = l -> l.stream().filter(i -> i.getMessage().length() < 100).collect(Collectors.toCollection(ArrayList::new));
        LogProcessor<T> thirdLogProcessor = l -> l.stream().sorted().collect(Collectors.toCollection(ArrayList::new));
