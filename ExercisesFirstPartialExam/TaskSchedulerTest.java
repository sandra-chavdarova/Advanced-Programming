...
class Schedulers {
    public static <T extends Task> TaskScheduler<T> getOrdered() {
        TaskScheduler<T> task = new TaskScheduler<T>() {
            @Override
            public List<T> schedule(T[] tasks) {
                List<T> newList = new ArrayList<>(Arrays.asList(tasks));
    ...
    public static <T extends Task> TaskScheduler<T> getFiltered(int order) {
        TaskScheduler<T> t = new TaskScheduler<T>() {
            @Override
            public List<T> schedule(T[] array) {
                List<T> newList = new ArrayList<>();
                for (int i = 0; i < array.length; i++) {
                    if (array[i].getOrder() < order) {
                        newList.add(array[i]);
                    }
                }
                return newList;
