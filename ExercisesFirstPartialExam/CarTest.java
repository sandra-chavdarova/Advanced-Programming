...
class Car implements Comparable<Car> {
    ...
    public int compareTo(Car o) {
        int comparison = Integer.compare(this.price, o.price);
        if (comparison == 0) {
...
class CarCollection {
    ...
    public List<Car> filterByManufacturer(String manufacturer) {
        return cars.stream().sorted(Comparator.comparing(Car::getModel)).filter(car -> car.getManufacturer().equalsIgnoreCase(manufacturer)).collect(Collectors.toList());
    }
